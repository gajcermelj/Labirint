window.onload = function() {
    ...
    var piratSize = 40;
    piratImg.onload = function() {
        piratFrameW = piratImg.width / 4;
    ...
    function processMovement() {
        ...
        if (dx !== 0 && dy !== 0) {
            ...
            for (var s = 0; s < speed; s++) {
                ...
                } else if (canMoveRect(x + dx, y)) {
                    x += dx; moved = true;
                } else if (canMoveRect(x, y + dy)) {
                    y += dy; moved = true;
                }
            }
            if (moved) checkCoinPickup();
        } else if (dx !== 0 || dy !== 0) {
            var moved = false;
            for (var s = 0; s < speed; s++) {
                if (canMoveRect(x + dx, y + dy)) {
    ...
    function checkCoinPickup() {
        for (var i = 0; i < coins.length; i++) {
            if (!coins[i].taken && x < coins[i].x + 10 && x + velikost > coins[i].x && y < coins[i].y + 10 && y + velikost > coins[i].y) {
    ...
    function drawFootprints() {
        ...
        for (var i = 0; i < tocke.length; i++) {
            ctx.beginPath();
            ctx.arc(tocke[i][0], tocke[i][1], 2, 2, Math.PI * 2);
            ctx.fill();
        }
    }
