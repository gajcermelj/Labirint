...
coinImg.src = 'slike/coin_gold.png';
var coinFrames = 8;
var coinFrameWidth = 32;
var coinFrameHeight = 32;
var currentCoinFrame = 0;
var coinAnimCounter = 0;
var coinAnimSpeed = 6;
var hintUnlocked = false;
var showHint = false;

// 2. PRIPRAVA NEVIDNEGA PLATNA (Narišemo samo črne zidove)
function prepareCollisionMap() {
    ...
    for (var i = 0; i < mazePathX1.length; i++) {
        ...
        collisionCtx.moveTo(mazePathX1[i], mazePathY1[i]);
        collisionCtx.lineTo(mazePathX2[i], mazePathY2[i]);
        collisionCtx.stroke();
    }
}

// 3. NOVO PREVERJANJE TRKOV (Gledamo nevidno platno)
function hitsWallAt(px, py) {
  ...
  // Preverimo, če je piksel na nevidnem platnu črn
...
function canMoveRect(newX, newY) {
  ...
  for (var j = 0; j <= velikost; j += sample) {
    if (hitsWallAt(newX, newY + j) || hitsWallAt(newX + velikost, newY + j)) return false;
  }
  return true;
}

// Ostale funkcije (Isto kot prej)
function spawnCoins() {
...
var chaserX = tocke[0][0] - 5, chaserY = tocke[0][1] - 5, chaserIndex = 0;

setInterval(() => { if(igraPoteka && !gameOver && !gameWin) totalSeconds++; 
    var m = Math.floor(totalSeconds/60), s = totalSeconds%60;
    timerElement.textContent = (m<10?"0":"")+m+":"+(s<10?"0":"")+s;
}, 1000);

setInterval(() => { if(igraPoteka && !gameOver && !gameWin && !chaserActive) {
    chaseIn--; countdownElement.textContent = chaseIn <= 0 ? "Lovec: AKTIVEN!" : "Lovec čez: " + chaseIn;
    if(chaseIn <= 0) chaserActive = true;
}}, 1000);

function moveChaser() {
    if (!chaserActive || chaserIndex >= tocke.length) return;
    var tx = tocke[chaserIndex][0]-5, ty = tocke[chaserIndex][1]-5;
    if (chaserX < tx) chaserX++; else if (chaserX > tx) chaserX--;
    if (chaserY < ty) chaserY++; else if (chaserY > ty) chaserY--;
    if (Math.abs(chaserX-tx)<1 && Math.abs(chaserY-ty)<1) chaserIndex++;
}

// GLAVNA ZANKA
function update() {
  coinAnimCounter++;
  if (coinAnimCounter >= coinAnimSpeed) {
    currentCoinFrame = (currentCoinFrame + 1) % coinFrames;
    coinAnimCounter = 0;
  }
  ...
  drawCoins();
  if (igraPoteka) moveChaser();
  if (chaserActive) { ctx.fillStyle="purple"; ctx.fillRect(chaserX, chaserY, 10, 10); }
  drawPlayer();

  if (igraPoteka && !gameOver && !gameWin) {
    if (chaserActive && rectsOverlap(x,y,10,chaserX,chaserY,10)) gameOver = true;
    if (Math.hypot(x+5-finishX, y+5-finishY) < 12) gameWin = true;
  }

  if ((gameOver || gameWin) && !obvestiloPrikazano) {
    obvestiloPrikazano = true;
    Swal.fire({ title: gameWin?'ZMAGA!':'KONEC', icon: gameWin?'success':'error' }).then(()=>location.reload());
  }
  requestAnimationFrame(update);
}

// Start
hintBtn.onclick = () => { showHint = !showHint; };
Swal.fire({ title: 'Začni?', confirmButtonText: 'Gremo!' }).then(() => { 
    prepareCollisionMap(); // Pripravimo nevidno platno
    spawnCoins(); 
    igraPoteka = true; 
    update(); 
});